...
class Client(ClientBase):
    ...
    def __init__(self, triad, uid, model, device, config: CfgNode, batch_size=32) -> None:
        """客户端调用fit进行训练
        ...
        super().__init__(device, model, config)
        self.triad = triad
        self.uid = uid
        self.loss_list = []
        self.n_item = len(triad)  # 该用户对应的项目数量
...
class Clients(ClientsBase):
    ...
    def __init__(self, triad, model, device, config: CfgNode) -> None:
        ...
        # self.config = config
        super().__init__(triad, model, device, config)
