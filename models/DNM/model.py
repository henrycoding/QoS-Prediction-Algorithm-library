...
class DNM(nn.Module):
    def __init__(self, config):
        ...
        self.UID_embedding = nn.Embedding(339, self.embed_dim)
        self.UR_embedding = nn.Embedding(31, self.embed_dim)
        self.UAS_embedding = nn.Embedding(137, self.embed_dim)
        self.SID_embedding = nn.Embedding(5825, self.embed_dim)
        self.SR_embedding = nn.Embedding(74, self.embed_dim)
        self.SAS_embedding = nn.Embedding(993, self.embed_dim)

        self.perception_layer = nn.ModuleList()
        layers = self.config.perception_layers
        for in_size, out_size in zip(layers[:-1], layers[1:]):
            self.perception_layer.append(nn.Linear(in_size, out_size))

        self.task_specific_layer_rt = nn.ModuleList()
        layers = self.config.task_specific_layers_rt
        for in_size, out_size in zip(layers[:-1], layers[1:]):
            self.task_specific_layer_rt.append(nn.Linear(in_size, out_size))
        self.affine_output_rt = nn.Linear(layers[-1], 1)
        self.logistic_rt = nn.Sigmoid()

        self.task_specific_layer_tp = nn.ModuleList()
        layers = self.config.task_specific_layers_tp
        for in_size, out_size in zip(layers[:-1], layers[1:]):
            self.task_specific_layer_tp.append(nn.Linear(in_size, out_size))
    ...
    def forward(self, user_indices, service_indices):
        ...
        add_vec = uid_embedding + sid_embedding
        # add_vec = None
        # for i in range(0, 6):
        #     if add_vec is None:
        #         add_vec = x_embed[i]
        ...
        for layer in self.perception_layer:
            vector = layer(vector)
            vector = F.relu(vector)

        vector_rt = vector
        for layer in self.task_specific_layer_rt:
        ...
        vector_tp = vector
        for layer in self.task_specific_layer_tp:
